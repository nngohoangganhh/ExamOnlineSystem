...
public class UserService {
    ...
    public PageResponse<UserResponseDto> searchUsers(UserSearchRequest search, int pageNo, int pageSize) {
        ...
        // Guard: nếu trang không có user nào (vd filter theo ngày quá hẹp),
        // KHÔNG gọi findWithRolesByIdIn với list rỗng — tránh sinh SQL "IN ()"
        // hoặc lỗi tùy phiên bản Hibernate.
        java.util.Map<Long, User> withRoles = userIds.isEmpty()
                ? Collections.emptyMap()
                : userRepository.findWithRolesByIdIn(userIds).stream()
        ...
        if (isStudent) {
            ...
            if (request.getStudentCode() == null
                ...
                throw new ResponseStatusException(
                        ...
                        "Student bắt buộc phải có studentCode."
                );
            }
            ...
            if (request.getEmployeeCode() != null
                ...
                throw new ResponseStatusException(
                        HttpStatus.BAD_REQUEST,
                        "Student không được nhập employeeCode."
                );
            }
        } else {
            ...
            if (request.getEmployeeCode() == null
                ...
                throw new ResponseStatusException(
                        HttpStatus.BAD_REQUEST,
                        "Employee bắt buộc phải có employeeCode."
                );
            }
            ...
            if (request.getStudentCode() != null
                ...
                throw new ResponseStatusException(
                        HttpStatus.BAD_REQUEST,
                        "Employee không được nhập studentCode."
                );
            }
